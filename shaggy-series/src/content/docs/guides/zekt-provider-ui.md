...
In order to be able to choose the "Provider" item on the left hand side of the management console (A) - you need to either be a "Provider" or "Both" (meaning Provider & Consumer) - which is an aossicated property of your profile. This property steers access to the navigation tab, as it only contain functionality relevant if you are a "provider" or "provider and consumer". [Please re-visist Zekt documentation, read Zekt personas in order to make an informed decisions if there are concerns](../overview/zekt-personas.md).
