---
title: Zekt Consumer repositories
description: Guide - Handle your Zekt consumer repositories
---

## - Zekt Repos (Consumer)

The management console (UI) - is having persona based navigation items. Second one, being the "Consumer" item. As soon as you click on it, you will be taken to the "Zekt Repos" tab (for the consumer persona). In this view - you will be able to enable / disable repositories for which you are in control of. The enablement is about instrumenting the repositories with the required Zekt Orchestration App (approving it) and hooking it up to Zekt services. Likewise, you can disable a Zekt repository from this UI - when the repository is no longer needed.

Zekt is per-repository based fees (flat rate - 10 USD / repo / month) + dynamic costs!


### - Enable Zekt repository (first time enablement scenario)

This section shows the basic steps (and verification steps) to perform as a customer, when enabling a Zekt consumer repository. Follow sequence below:
...
2. Once you toggle the switch to enable repository - a pop-up will appear, which informs you that you will need to approve and install the ["Zekt Orchestrator App"](/zekt-docs/overview/zekt-apps/) on the target consumer repository in order to have it enabled. Press "Continue to Github" button - to proceed. Pop-up modal shown below:

![Inform user - enable consumer repository](../../../assets/onboard-first-time-provider-repo-modal.png)
...
![Processing Zekt consumer repo enablement](../../../assets/process-onboarding-provider-repo.png)
...
![Enabled Zekt consumer repoistory](../../../assets/provider-repo-enabled.png)
